...
/**
 * {@link org.springframework.web.servlet.HandlerExceptionResolver} of tiles for exception handler
 *
 * @author David Hsing
...
public class TilesFilterExceptionResolver extends DefaultFilterExceptionResolver {
    private final TilesViewResolver viewResolver;
    ...
    protected void resolveHtmlInternal(@Nonnull HttpServletRequest request, @Nonnull HttpServletResponse response, @Nullable Object handler, @Nonnull Exception cause, @Nullable ModelAndView view) {
        ...
        if (view != null && StringUtils.isNotBlank(view.getViewName())) {
            TilesView resolvedView = (TilesView) viewResolver.resolveViewName(view.getViewName(), LocaleContextHolder.getLocale());
